...
import { popularRoutes } from "@/constants";
...
const PopularCarouselSection: React.FC = () => {
  ...
  const handleNext = (): void => {
    setCurrentIndex((prev) => {
      if (prev >= popularRoutes.length - itemsPerView) {
        return 0;
      }
      return prev + 1;
    });
  ...
  const handlePrev = (): void => {
    setCurrentIndex((prev) => {
      if (prev <= 0) {
        return popularRoutes.length - itemsPerView;
      }
      return prev - 1;
    });
  };
...
export default PopularCarouselSection;
